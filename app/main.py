...
def _queue_has_push_subscription(endpoint: str, user_id: int | None = None) -> bool:
    ...
    for line in reversed(lines[-200:]):
        try:
            ...
            continue
        if (record.get('endpoint') or '').strip() != endpoint:
            continue
        if user_id is not None and int(record.get('user_id') or 0) != int(user_id):
            continue
        return True
    return False
...
MIND_ROUTINE_SECTIONS = [
    {
        ...
        "subtitle": "기준부터 바로잡기",
        "checks": [
            {"code": "P01", "label": "목표보다 시스템이 중요하다는 기준을 다시 확인했다"},
            {"code": "P02", "label": "오늘은 하루 1개 집중 원칙으로 움직이기로 정했다"},
        ],
    },
    {
        "code": "morning",
        "icon": "🌅",
        "title": "1. 아침 시작 체크",
        "subtitle": "출근 직후 / 기상 후 10분",
        "checks": [
            {"code": "M01", "label": "오늘 가장 중요한 1가지를 확인했다"},
            {"code": "M02", "label": "다른 할 일은 전부 무시하겠다고 선언했다"},
            {"code": "M03", "label": "PC와 작업툴을 바로 실행해 환경을 세팅했다"},
            {"code": "M04", "label": "폰과 알림 등 방해 요소를 껐다"},
        ],
    },
    {
        "code": "focus",
        ...
        "subtitle": "핵심 집중 시간",
        "checks": [
            {"code": "F01", "label": "한 가지 일만 하며 멀티태스킹을 끊었다"},
            {"code": "F02", "label": "최소 60~120분 몰입 구간을 지켰다"},
            {"code": "F03", "label": "딴생각은 메모만 하고 계속 진행했다"},
            {"code": "F04", "label": "SNS, 다른 일 시작, 계획 수정을 끊었다"},
        ],
    },
    {
        "code": "speed",
        ...
        "subtitle": "5분 룰",
        "checks": [
            {"code": "S01", "label": "아이디어가 떠오르면 5분 안에 행동 1개를 실행했다"},
            {"code": "S02", "label": "생각만 하지 않고 메모·연락·검색 중 하나로 바로 옮겼다"},
        ],
    },
    {
        "code": "money",
        ...
        "subtitle": "비즈니스 필수",
        "checks": [
            {"code": "B01", "label": "고객과 연결되는 행동을 최소 1개 실행했다"},
            {"code": "B02", "label": "매출 가능성이 있는 행동을 최소 1개 실행했다"},
            {"code": "B03", "label": "영업 연락·콘텐츠 업로드·상품 개선 중 하나 이상 실행했다"},
        ],
    },
    {
        "code": "environment",
        ...
        "subtitle": "흐름 끊기지 않게",
        "checks": [
            {"code": "E01", "label": "필요한 것만 눈앞에 두었다"},
            {"code": "E02", "label": "귀찮고 흐름을 끊는 요소를 제거했다"},
            {"code": "E03", "label": "작업 흐름이 끊기지 않도록 유지했다"},
        ],
    },
    {
        "code": "evening",
        ...
        "subtitle": "10분 필수",
        "checks": [
            {"code": "N01", "label": "오늘 떠오른 생각을 전부 기록했다"},
            {"code": "N02", "label": "해야 할 일을 전부 적어 뇌를 비웠다"},
            {"code": "N03", "label": "내일 가장 중요한 1개를 선택했다"},
        ],
    },
    {
        "code": "reset",
        ...
        "subtitle": "성공 루틴 핵심",
        "checks": [
            {"code": "R01", "label": "책상을 정리했다"},
            {"code": "R02", "label": "작업 화면을 다시 세팅했다"},
            {"code": "R03", "label": "필요한 자료를 미리 열어두었다"},
        ],
    },
    {
        "code": "direction",
        ...
        "subtitle": "중요",
        "checks": [
            {"code": "D01", "label": "지금 하는 일이 성장과 연결되는지 점검했다"},
            {"code": "D02", "label": "그냥 바쁜 일인지 아닌지 구분했다"},
        ],
    },
    {
        "code": "mission",
        ...
        "subtitle": "포기 방지 핵심",
        "checks": [
            {"code": "I01", "label": "나는 누구를 돕고 있는지 다시 확인했다"},
            {"code": "I02", "label": "이 일이 누군가에게 의미 있는 일인지 점검했다"},
        ],
    },
    {
        "code": "result",
        "icon": "📊",
        "title": "10. 결과 집착 금지 체크",
        "subtitle": "반드시 체크",
        "checks": [
            {"code": "G01", "label": "결과에 끌려가지 않고 행동만 체크했다"},
            {"code": "G02", "label": "나는 지금 씨앗을 심는 중이라고 다시 상기했다"},
        ],
    },
]
